...
user = User.where(email: 'user@gift.com').first
user = User.create! email: 'user@gift.com', password: 'qweasd', name: 'User', confirmed_at: Time.now if user.nil?

def seed_image(file_name)
  File.open(File.join(Rails.root, "/db/assets/images/#{file_name}.jpg"))
end

def create_seeds user
  project_count = Project.where(name: 'New chessboard for our office').count
